...
# Download dataset of interest at the same level of aggregation
# Replace "https://example.com/dataset.csv" with the URL of your dataset
dataset <- read.csv("https://example.com/dataset.csv")

# Check the structure of the shapefile and dataset
print(head(nuts2_shapefile))
print(head(dataset))
...
# Plot the dataset on the map
ggplot() +
  geom_sf(data = nuts2_shapefile) +
  geom_point(data = dataset, aes(x = longitude_column, y = latitude_column)) +  # Replace with appropriate column names
  theme_minimal()
